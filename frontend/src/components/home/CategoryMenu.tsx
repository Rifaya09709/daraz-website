...
// Maps every campaign category chip label to the actual `category`
// (and, where needed, `subCategory`) values stored in the backend DB.
//
// Keys are CAMPAIGN-SCOPED ("slug:label") wherever the same label means
// different things in different campaigns (e.g. "Accessories" = phone
// accessories in the mobiles campaign, but handbags/backpacks in the
// fashion campaign). A plain-label key still works as a shared fallback
// for labels that mean the same thing everywhere (e.g. "Fashion",
// "Electronics").
//
// Labels with NO entry here at all (promo-only concepts like
// "Free Delivery", "Live Shopping", "Choice Deals" etc.) are handled by
// the fallback in the effect below — they show latest products instead
// of an empty grid.
const categoryLabelToDbCategory: Record<string, { category?: string; subCategory?: string }> = {
  // ---- campaign-scoped overrides (resolve conflicts) ----
  "mobiles:Accessories": { category: "Electronics", subCategory: "Electronics Accessories" },
  "fashion:Accessories": { category: "Fashion", subCategory: "Bags" },

  // mobiles campaign
  Smartphones: { category: "Mobiles", subCategory: "Smartphones" },
  Smartwatches: { category: "Electronics", subCategory: "Watches" },
  "Trade-In": { category: "Mobiles" },

  // mall campaign
  Electronics: { category: "Electronics" },
  Fashion: { category: "Fashion" },
  Beauty: { category: "Beauty & Makeup" },
  "Home & Living": { category: "Furniture" },

  // beauty campaign
  Skincare: { category: "Beauty & Makeup" },
  Makeup: { category: "Beauty & Makeup" },
  Haircare: { category: "Personal Care", subCategory: "Hair Styling Tools" },
  Fragrance: { category: "Beauty & Makeup", subCategory: "Fragrance" },

  // fashion campaign
  Women: { category: "Fashion" },
  Men: { category: "Fashion" },
  Kids: { category: "Kids" },

  // free-delivery campaign
  FMCG: { category: "Groceries" },
  Lifestyle: { category: "Home Essentials" },

  // low-price campaign
  "Kitchen & Dining": { category: "Home & Kitchen" },
  "Tools, DIY & Outdoor": { category: "Home Essentials" },
  "Health & Beauty": { category: "Beauty & Makeup" },

  // new-arrivals campaign
  Gadgets: { category: "Electronics" },
  "Home Decor": { category: "Home Essentials" },
};

// campaignSlug is checked first so the same label can resolve differently
// per campaign; falls back to the shared plain-label entry; returns
// undefined if nothing matches (handled by the caller as "no filter").
const resolveDbCategory = (campaignSlug: string, label: string) =>
  categoryLabelToDbCategory[`${campaignSlug}:${label}`] ||
  categoryLabelToDbCategory[label];
